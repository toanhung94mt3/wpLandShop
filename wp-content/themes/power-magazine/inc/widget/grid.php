...
function power_magazine_action_grid() {

  register_widget( 'Power_magazine_Grid' );
  
}
add_action( 'widgets_init', 'power_magazine_action_grid' );
class Power_magazine_Grid extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_ops = array(
		  'classname'   => 'business-section',
		  'description' => esc_html__( 'Add Widget to Display Grid Style Widget.', 'power-magazine' )
		);
		parent::__construct( 'power_magazine_grid',esc_html__( 'PM: Grid Style', 'power-magazine' ), $widget_ops, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, 
			array( 
			  'title'			=> esc_html__( 'Sports', 'power-magazine' ),		
			  'category'       	=> '', 
			  'number'          => 4, 
			  'show_post_meta'	=> true,
			  'show_category'	=> true,

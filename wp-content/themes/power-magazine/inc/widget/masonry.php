<?php
/**
 * Masonry Style Widget.
 *
 * @package Power_Magazine
 */

function power_magazine_action_masnory() {

  register_widget( 'Power_magazine_Masonry' );
  
}
add_action( 'widgets_init', 'power_magazine_action_masnory' );
class Power_magazine_Masonry extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_ops = array(
		  'classname'   => 'fashion-section',
		  'description' => esc_html__( 'Add Widget to Display Masonry Style Widget.', 'power-magazine' )
		);
		parent::__construct( 'power_magazine_action_masnory',esc_html__( 'PM: Masonry Style', 'power-magazine' ), $widget_ops, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, 
			array( 
			  'title'			=> esc_html__( 'Fashion', 'power-magazine' ),		
			  'category'       	=> '', 
			  'number'          => 4, 
			  'show_post_meta'	=> true,
			  'show_category'	=> true,
        ...
        if ($the_query->have_posts()) : $count= 0; ?>		            
            <div class="grid absolute-content">
            	<?php while ( $the_query->have_posts() ) : $the_query->the_post(); $count++;
            		$image_size = 'power-magazine-masonry-large';
            		if($count % 2 == 0){ 
